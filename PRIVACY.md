...
- 若用户已经授予辅助功能权限，运行中的面板会查找 Codex 暴露的固定辅助功能标签“打开通知 / Open notification”和“静音任务 / Mute task”，并调用对应的菜单动作关闭新任务气泡。读取到的其他辅助功能字符串不保留、不记录、不上传；面板不生成鼠标或键盘事件，也不会主动弹出授权请求。未授权时只检查授权状态，不访问 Codex 的辅助功能树、不执行菜单动作，仍在 Codex 完全退出后同步任务 ID。
...
- 辅助功能：可选，只用于在当前 Codex 进程中自动执行其自带的“静音任务”菜单动作；未授权时额度、任务面板和退出后的气泡静音同步仍正常工作。
