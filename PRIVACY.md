...
- 本机集成管理只支持 Codex、Claude Code、Cursor、ZCode 和 Pi。Codex 集成固定为 `notManaged`，其 install/repair/uninstall 路径不会写 Codex 配置；Claude、Cursor、ZCode 或 Pi 未安装时，不会为该 Agent 凭空创建受管配置。
- Claude Code 只在 `~/.claude/settings.json` 管理自己的 `PermissionRequest` Hook；Cursor 只在 `~/.cursor/hooks.json` 管理带所有权标记的生命周期 Hook；ZCode 只在 `~/.zcode/cli/config.json` 管理状态观察 Hook；Pi 只管理 `~/.pi/agent/extensions/threadhelm-state-observer/`。所有非 ThreadHelm 条目和显式禁用设置都会保留。
...
- 卸载器只移除 ThreadHelm 拥有的 Claude、Cursor、ZCode 和 Pi 条目，保留其他配置；测试使用临时隔离 root，不读取或写入真实厂商配置。
