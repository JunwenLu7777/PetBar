...
ThreadHelm 是一个独立运行的 macOS App。当前发行版本为 **1.1.2**，输出文件名为：

```text
ThreadHelm-macOS-arm64-1.1.2.zip
```
...
1. 完整解压 `ThreadHelm-macOS-arm64-1.1.2.zip`。
...
构建成功后会生成 `dist/ThreadHelm-macOS-arm64-1.1.2.zip`，并在 staging 目录内生成 `CHECKSUMS-SHA256.txt`。
