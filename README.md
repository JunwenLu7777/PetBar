...
ChatBird 是一个独立运行的 macOS App。当前发行版本为 **1.1.0**，输出文件名为：
...
ChatBird 的源码与发布只以 [JunwenLu7777/PetBar](https://github.com/JunwenLu7777/PetBar) 为准。
...
6. 安装完成后会复制到 `~/Applications/ChatBird.app`，并启动 `dev.chatbird.app`。
7. 若希望当前 Codex 运行中新建任务的原生气泡也自动静音，可在“系统设置 → 隐私与安全 → 辅助功能”中为“ChatBird”开启权限；未开启不影响核心功能。
...
卸载器会移除 `~/Applications/ChatBird.app`、`dev.chatbird.app` LaunchAgent、ChatBird 日志和健康缓存，并只移除 ChatBird 记录过的 Codex 原生气泡静音值。卸载前请先完全退出 Codex；若 Codex 仍在运行，卸载器会停止且保留当前安装与恢复文件。
...
构建成功后会生成 `dist/ChatBird-macOS-arm64-1.1.0.zip`，并在 staging 目录内生成 `CHECKSUMS-SHA256.txt`。
...
原创代码使用 MIT License。ChatBird 视觉资产、图像和预览文件的授权边界见 [ASSET-NOTICE.md](ASSET-NOTICE.md)，本机数据处理边界见 [PRIVACY.md](PRIVACY.md)。
