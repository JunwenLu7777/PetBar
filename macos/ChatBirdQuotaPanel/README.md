...
- 已授予辅助功能权限时，ChatBird 约每 2 秒把带有 `Show activity, N item(s)` / `显示活动，N 项` 按钮的 Codex 计数角标窗口移到所有活动显示器之外，并继续通过 Codex 自带的“静音任务”菜单关闭新任务气泡；同名的任务列表窗口不会移动。该过程不会移动鼠标或发送按键，只匹配固定辅助功能标签，其他字符串不保留、不记录、不上传。未授权时仍可手动静音，退出后会加入下次启动使用的静音列表。
...
- 不发起远程第三方行情请求。
- 不修改 Codex 应用本体，Codex 更新不会覆盖 ChatBird。
...
Codex 完全退出后，ChatBird 会短暂等待全局状态与任务索引稳定，再合并静音任务 ID；如果 Codex 在等待期间重新启动，本次同步会取消并留到下次退出后执行。

当前开发构建位置：`./build/ChatBird.app`
安装位置：`~/Applications/ChatBird.app`

## 数据自检

```bash
./build/ChatBird.app/Contents/MacOS/ChatBird --print-quota
./build/ChatBird.app/Contents/MacOS/ChatBird --print-claude-quota
./build/ChatBird.app/Contents/MacOS/ChatBird --print-task-progress
./build/ChatBird.app/Contents/MacOS/ChatBird --print-panel-location
./build/ChatBird.app/Contents/MacOS/ChatBird --print-saved-panel-location
./build/ChatBird.app/Contents/MacOS/ChatBird --self-test-placement
./build/ChatBird.app/Contents/MacOS/ChatBird --self-test-lifecycle
./build/ChatBird.app/Contents/MacOS/ChatBird --self-test-native-notification-state
./build/ChatBird.app/Contents/MacOS/ChatBird --self-test-task-progress
./build/ChatBird.app/Contents/MacOS/ChatBird --self-test-weekly-quota
./build/ChatBird.app/Contents/MacOS/ChatBird --self-test-claude-quota
./build/ChatBird.app/Contents/MacOS/ChatBird --self-test-chatbird-edition
./build/ChatBird.app/Contents/MacOS/ChatBird --self-test-dynamic-island
```

灵动岛确定性预览：

```bash
./build/ChatBird.app/Contents/MacOS/ChatBird \
...
卸载前必须完全退出 Codex；若仍在运行，脚本会停止且不会移除 App 或原生气泡恢复文件。卸载器会移除 `~/Applications/ChatBird.app`、`dev.chatbird.app` LaunchAgent、ChatBird 日志和健康缓存，并只恢复本项目记录过的 Codex 原生气泡设置。
