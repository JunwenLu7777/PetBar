...
- Apple 芯片或 Intel Mac
- 已安装并登录 Codex
- Claude Code 额度与任务兼容为可选功能；使用时需安装并登录 Claude Code
...
- 左侧下拉框可切换额度来源：Codex 显示周额度，Claude Code 同时显示 5h、周额度与 Fable 周额度；不显示 Token、Credits 或任何行情模块。
- 任务状态约每 2 秒读取本机 Codex 与 Claude Code 会话状态，并显示执行中、等待确认、已完成和执行失败；点击 Codex 任务会通过官方深链打开，点击 Claude 任务会在原工作目录通过 Terminal 恢复会话。
