...
- Antigravity hooks are registered only under `~/.gemini/antigravity-cli/`, the CLI-specific customization root. The cross-product `~/.gemini/config/` root also loads, but installing there would fire our hooks inside IDE sessions that ThreadHelm cannot attribute.
